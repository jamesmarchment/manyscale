...
export async function resolveTableIDs(tenant) {
  ...
  if (!measuresTable) {
    console.error(
      `[resolveTableIDs] Table "Measures" not found in base ${baseId}. ` +
  ...
  if (!translationsTable) {
    console.warn(
      `[resolveTableIDs] Table "Translations" not found in base ${baseId}. ` +
  ...
  if (!contributorsTable) {
    console.warn(
      `[resolveTableIDs] Table "Contributors" not found in base ${baseId}. ` +
  ...
  if (secondaryPat && secondaryBaseId) {
    ...
    try {
      ...
      if (!res2.ok) {
        ...
        if (!mt2) {
          console.warn(`[resolveTableIDs] "Measures" not found in secondary base ${secondaryBaseId} — secondary source disabled.`);
        } else {
          secondaryMeasureTableIDs.set(tenant.slug, mt2.id);
          console.log(`Resolved secondary Measures table: ${mt2.id}`);
          if (tt2) {
            secondaryTranslationTableIDs.set(tenant.slug, tt2.id);
            console.log(`Resolved secondary Translations table: ${tt2.id}`);
          }
        }
      }
    } catch (err) {
...
export async function refreshCache(slug) {
  ...
  if (!tenant) {
    console.error(`[refreshCache] No tenant found for slug "${slug}"`);
    return;
  }
  const pat = process.env[tenant.patEnvVar];
  const baseId = tenant.baseId;
  ...
  let allRecords;
  try {
    allRecords = await fetchAllPages(pat, baseId, MEASURES_TABLE_ID, r => r.fields["Status"] === "Approved");
  ...
  if (secondaryMeasureTableID && secondaryPat && secondaryBaseId) {
    ...
    try {
      const secondary = await fetchAllPages(secondaryPat, secondaryBaseId, secondaryMeasureTableID, r => r.fields["Status"] === "Approved");
  ...
  if (TRANSLATIONS_TABLE_ID) {
    ...
    try {
      const trs = await fetchAllPages(pat, baseId, TRANSLATIONS_TABLE_ID);
  ...
  if (secondaryTranslationTableID && secondaryPat && secondaryBaseId) {
    ...
    try {
      const trs2 = await fetchAllPages(secondaryPat, secondaryBaseId, secondaryTranslationTableID);
