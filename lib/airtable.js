...
const SCAFFOLD_TABLE_FIELDS = {
  Measures: [
    ...
    // Data-entry-only fields — not read anywhere in the app, but used by uploaders to
    // track provenance and permissions while populating a base.
    { name: "Author Name", type: "singleLineText" },
    { name: "Author Email", type: "email" },
    { name: "Author Permission", type: "checkbox", options: { icon: "check", color: "greenBright" } },
    { name: "Preferred", type: "number", options: { precision: 0 } },
    { name: "Secondary References (Optional)", type: "multilineText" },
    { name: "R Code For Scoring the Measure (Optional)", type: "multipleAttachments" },
    // "Contributed By" (link) and "Name (from Contributed By)" (lookup through it) are
    // appended after Contributors exists — see addContributedByFields().
  ],
  Translations: [
    ...
    { name: "Status", type: "singleSelect", options: { choices: [{ name: "Approved" }, { name: "Pending" }, { name: "Rejected" }] } },
    { name: "Final PDF", type: "multipleAttachments" },
    // Data-entry-only fields, mirroring the Measures-level equivalents above.
    { name: "Measure Upload", type: "multipleAttachments" },
    { name: "Translation Citation", type: "multilineText" },
    { name: "Copyright (Required)", type: "multilineText" },
    { name: "Author Permission", type: "checkbox", options: { icon: "check", color: "greenBright" } },
    // "Contributed By" / "Name (from Contributed By)" appended by addContributedByFields().
  ],
  Contributors: [
    { name: "Name", type: "singleLineText" },
...
export async function scaffoldTenantTables(pat, baseId) {
  ...
  await addContributedByFields(pat, baseId, metaUrl, summary);

  return summary;
}

// "Contributed By" (a real link to Contributors) and "Name (from Contributed By)" (a
// genuine lookup through that link) can't live in SCAFFOLD_TABLE_FIELDS above — they need
// the Contributors table's id and its "Name" field's id, neither of which exist until
// Contributors itself has been created. Re-reads the base's tables (cheap, and simplest
// way to get accurate field ids whether each table was just created or already existed)
// and adds whichever of the two fields are still missing from Measures/Translations.
// Never throws — failures are appended to the passed-in summary instead.
async function addContributedByFields(pat, baseId, metaUrl, summary) {
  let tables;
  try {
    const response = await fetch(metaUrl, { headers: { Authorization: `Bearer ${pat}` } });
    if (!response.ok) {
      summary.errors.push({ table: null, message: `Could not re-read tables for "Contributed By" linking (status ${response.status}).` });
      return;
    }
    tables = (await response.json()).tables || [];
  } catch (err) {
    summary.errors.push({ table: null, message: `Could not re-read tables for "Contributed By" linking: ${err.message}` });
    return;
  }

  const contributors = tables.find(t => t.name === "Contributors");
  const contributorsNameField = contributors && contributors.fields.find(f => f.name === "Name");
  if (!contributors || !contributorsNameField) {
    summary.errors.push({ table: "Contributors", message: `Could not find Contributors table or its "Name" field — skipped "Contributed By" linking.` });
    return;
  }

  for (const tableName of ["Measures", "Translations"]) {
    const table = tables.find(t => t.name === tableName);
    if (!table) continue; // table itself failed to create earlier; already reported above

    try {
      let linkField = table.fields.find(f => f.name === "Contributed By");
      if (!linkField) {
        const createLinkResponse = await fetch(`https://api.airtable.com/v0/meta/bases/${baseId}/tables/${table.id}/fields`, {
          method: "POST",
          headers: { Authorization: `Bearer ${pat}`, "Content-Type": "application/json" },
          body: JSON.stringify({ name: "Contributed By", type: "multipleRecordLinks", options: { linkedTableId: contributors.id } }),
        });
        if (!createLinkResponse.ok) {
          const errBody = await createLinkResponse.text().catch(() => "");
          summary.errors.push({ table: tableName, message: `Could not create "Contributed By" link field (status ${createLinkResponse.status}): ${errBody}` });
          continue;
        }
        linkField = await createLinkResponse.json();
      }

      if (!table.fields.some(f => f.name === "Name (from Contributed By)")) {
        const createLookupResponse = await fetch(`https://api.airtable.com/v0/meta/bases/${baseId}/tables/${table.id}/fields`, {
          method: "POST",
          headers: { Authorization: `Bearer ${pat}`, "Content-Type": "application/json" },
          body: JSON.stringify({
            name: "Name (from Contributed By)",
            type: "multipleLookupValues",
            options: { recordLinkFieldId: linkField.id, fieldIdInLinkedTable: contributorsNameField.id },
          }),
        });
        if (!createLookupResponse.ok) {
          const errBody = await createLookupResponse.text().catch(() => "");
          summary.errors.push({ table: tableName, message: `Could not create "Name (from Contributed By)" lookup field (status ${createLookupResponse.status}): ${errBody}` });
        }
      }
    } catch (err) {
      summary.errors.push({ table: tableName, message: `Error linking "Contributed By": ${err.message}` });
    }
  }
}
