...
export async function refreshCache(slug) {
  ...
  // Table IDs are normally resolved ahead of time by the startup/interval refresh loop
  // (server.js), but a manual refresh (e.g. the architect "Refresh cache" button) can be
  // the first refresh since a restart — most commonly when AIRTABLE_REFRESH_ON_STARTUP is
  // off, so the interval hasn't fired yet. Resolve lazily here rather than failing, but
  // only when we don't already have them, so a normal already-resolved refresh doesn't
  // pay for an extra metadata API call it doesn't need.
  if (!measuresTableIDs.has(slug)) {
    console.log(`${pfx} Table IDs not yet resolved — resolving before refresh…`);
    await resolveTableIDs(tenant);
  }
