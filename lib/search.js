...
// Airtable long-text fields (e.g. "Primary Reference") sometimes contain literal
// embedded newlines mid-sentence. Collapsing all whitespace to single spaces makes
// matching robust to that, and — critically — keeps suggestion text newline-free so
// assigning it to an <input>.value never triggers the browser's single-line-input
// value-sanitization, which silently deletes embedded newlines (rather than replacing
// them with a space) and glues the surrounding words together into an unmatchable token.
...
export function recordMatchesSearch(record, query) {
  ...
  if (fields["Measure Name"] && normalizeWhitespace(fields["Measure Name"]).toLowerCase().includes(query)) return true;
  if (fields["Primary Reference"] && normalizeWhitespace(fields["Primary Reference"]).toLowerCase().includes(query)) return true;
  if (fields["Description of Measure"] && normalizeWhitespace(fields["Description of Measure"]).toLowerCase().includes(query)) return true;
  if (constructs.some(c => normalizeWhitespace(c).toLowerCase().includes(query))) return true;
...
export function getSuggestions(cache, query, { seen = new Set(), limit = 6 } = {}) {
  ...
  for (const record of cache) {
    ...
    for (const rawVal of candidates) {
      ...
      const val = normalizeWhitespace(rawVal);
