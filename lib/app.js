...
// Gzips every response above compression's default 1KB threshold — static vendor
// CSS/JS and the /api/data JSON payload both benefit, and the latter only grows as
// the tenant's dataset grows, so this matters more over time, not less.
app.use(compression());
app.use(cors());
...
//
// Vendor libraries only change via a manual developer file swap (e.g. d3.v7.min.js →
// d3-custom.min.js this week) — never overwritten in place at the same filename — so a
// longer cache window is safe here. Must be mounted before the general express.static
// below, which keeps the default always-revalidate behavior (max-age=0 + ETag/
// Last-Modified) for everything else: the site's own CSS/JS, per-tenant PDFs/photos, and
// the cache-stats.json/group-index.json/sitemap.xml files that get overwritten in place
// on every scheduled refresh.
app.use("/assets/vendor", express.static("public/assets/vendor", { maxAge: "7d" }));
...
app.use((err, req, res, next) => {
  ...
  // Catches everything that reaches this point without having already notified more
  // specifically upstream (a handful of /architect/tenants/:slug/* routes — toggle-active,
  // toggle-new, link — have no local try/catch around their writeJsonAtomic call, so a
  // write failure there lands here; so does any other uncaught exception). Deduped by
  // error name + path so a repeatedly-thrown error collapses into one row instead of one
  // per request.
  createNotification({
    scope: "architect", type: "server_error", severity: "error",
    message: `${err.name || "Error"} on ${req.method} ${req.path}: ${err.message}`,
    dedupeKey: `server_error:${err.name || "Error"}:${req.path}`,
  });
  // A response can already have gone out for this request by the time an error reaches
  // here — e.g. a session-store save/rename that fails asynchronously after the route
  // handler already sent its response (observed via session-file-store EPERM retries
  // under rapid same-session requests on this UNC-mounted project — a separate,
  // pre-existing race, not something this handler can fix). Calling res.send() again in
  // that case throws ERR_HTTP_HEADERS_SENT, which — since this is the last handler in
  // the chain — is otherwise an uncaught exception that crashes the whole process.
  // Delegating to next(err) here is Express's documented way to hand off to its built-in
  // final handler, which just closes the socket instead.
