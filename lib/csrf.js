...
// Public, anonymous, no-session form submissions (routes/forms.js, routes/landing.js)
// — each already has its own defense (antiSpamGuard's rate limit + honeypot timing),
// none of their pages render a CSRF token, and there's no privileged session for a
// forged cross-site POST to ride on. Matched by path suffix since these routes are
// mounted under a variable /:slug prefix in multi-tenant mode.
const CSRF_EXEMPT_SUFFIXES = ["/contact", "/suggest", "/request-repo"];
